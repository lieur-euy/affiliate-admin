...
import { tagApi, type Tag, type TagReq } from "@/api/tags"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const emptyForm: TagReq = { name: "" }

export function TagsPage() {
  const { t } = useTranslation()
  const [search, setSearch] = useState("")
  const fetcher = useCallback(
    (cursor?: string) => tagApi.list({ limit: 12, cursor, search: search || undefined }).then((r) => ({ data: r.data, cursor: r.cursor })),
    [search],
  )
  const {
  ...
  } = useCursorPagination<Tag>(fetcher, [search])
  useEffect(() => { loadFirst() }, [loadFirst])
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editing, setEditing] = useState<Tag | null>(null)
  const [form, setForm] = useState<TagReq>(emptyForm)
